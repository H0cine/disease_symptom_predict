...
data = pd.read_csv("symbipredict.csv")
...
def get_user_input(symptoms_list):
    user_input = []
    for i in range(0,len(symptoms_list) - 1):
        print("-",symptoms_list[i], " ")
    print("Please enter the symptoms you have (comma-separated, e.g., 'itching, fever, headache'):")
    user_input_str = input().lower()
    user_symptoms = [symptom.strip() for symptom in user_input_str.split(",")]
    binary_input = [1 if symptom in user_symptoms else 0 for symptom in symptoms_list]
...
user_input = get_user_input(symptoms_list)

predict_disease = predict_disease(user_input)

print(f"the predict disease is {predict_disease}")